...
class TransformerModel(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, features) 或 (batch, seq_len, features)
...
class TransformerTrainer:
    ...
    def __init__(self, input_size, output_size, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        if self.device == 'cuda':
            ...
            print(f"批次大小范围: 256-1024 (贝叶斯优化搜索)")
        else:
            print("⚠️  未检测到GPU，将使用CPU训练")
            print("建议: 安装CUDA版PyTorch以使用GPU加速")
        print(f"{'='*60}\n")
    
    def train_model(self, X_train, y_train, nhead, learning_rate, l2_reg, 
                   d_model=128, num_layers=2, batch_size=256, epochs=1000, 
    ...
    def bayesian_optimization(self, X_train, y_train, n_calls=15, verbose=True):
        ...
        """
        # 定义搜索空间
        # 注意：nhead 必须能整除 d_model (128)，所以只能选择 128 的约数 [2, 4, 8]
        space = [
            Categorical([2, 4, 8], name='nhead'),  # 限制为 128 的约数
    ...
    def save(self, filepath='models/transformer_model.pkl'):
        """保存模型"""
        import os
    ...
    @classmethod
    def load(cls, filepath='models/transformer_model.pkl', device='cuda' if torch.cuda.is_available() else 'cpu'):
        """加载模型"""
        ...
        # 确保所有参数都是Python原生类型
        d_model = int(checkpoint['best_params']['d_model'])
        nhead = int(checkpoint['best_params']['nhead'])
        num_layers = int(checkpoint['best_params']['num_layers'])
...
def plot_predictions(y_true, y_pred, n_outputs, dataset_name, metrics_dict, save_path=None):
    ...
    for i in range(n_outputs):
        ...
        ax.plot(x, y_pred[:, i], 'b-o', label='Bayes-Transformer预测值', linewidth=1, markersize=4)
...
def main():
    ...
    # 5. 保存模型
    print("\n[5] 保存模型...")
    trainer.save('models/transformer_model.pkl')
    ...
    train_pred_df.to_excel('results/transformer_train_predictions.xlsx', index=False)
    test_pred_df.to_excel('results/transformer_test_predictions.xlsx', index=False)
    ...
    plot_predictions(data_dict['y_train'], y_train_pred, 5, '训练集', 
                    train_metrics, 'results/figures/transformer_train_prediction.png')
    plot_predictions(data_dict['y_test'], y_test_pred, 5, '测试集', 
                    test_metrics, 'results/figures/transformer_test_prediction.png')
    
    plot_scatter(data_dict['y_train'], y_train_pred, 5, '训练集',
                train_metrics, 'results/figures/transformer_train_scatter.png')
    plot_scatter(data_dict['y_test'], y_test_pred, 5, '测试集',
                test_metrics, 'results/figures/transformer_test_scatter.png')
